...
void setup() {
  ...
  //usbMIDI.turnThruOn(midi::Thru::Mode::Off);
  ...
  oldVoices = Voices;
  midiCCOut(CCnumVoices, Voices, 16);
  //modAmount = getModAmount();
  saveMagicPatch = getSaveMagicPatch();
  addingPatch = getAddingPatch();
  usbNote = getUSBNote();
  usbBend = getUSBBend();
  usbMod = getUSBMod();
  usbChannel = getUSBChannel();
...
void updatevcf_frequency() {
  if (Detune == true) {
    ...
    midiCCOut((CCuniDetune), voice5_detune, 5);
  } else {
...
void updatevcf_resonance() {
  if (Detune == true) {
    ...
    midiCCOut((CCuniDetune), voice6_detune, 6);
  } else {
...
void updatepulsewidth() {
  if (Detune == true) {
    ...
    midiCCOut((CCuniDetune), voice2_detune, 2);
  } else {
...
void updatemodulation() {
  if (modAmount == true) {
    ...
    midiCCOut(CCmidiMod, midiModAmount, 1);
  } else {
...
void updateoscpwm() {
  if (Detune == true) {
    ...
    midiCCOut((CCuniDetune), voice3_detune, 3);
  } else {
...
void updateosc_2_freq() {
  if (Detune == true) {
    ...
    midiCCOut((CCuniDetune), voice4_detune, 4);
  } else {
...
void updateosc_1_freq() {
  if (Detune == true) {
    ...
    midiCCOut((CCuniDetune), voice1_detune, 1);
  } else {
...
void myMIDIChange(byte channel, byte control, byte value) {
  value = (value * 2);
...
void setCurrentPatchData(String data[]) {
  ...
  if (unison == 127) {
    if (voice1_detune < 1 || voice1_detune > 6) {
      midiCCOut(CCuniDetune, 1, 1);
    } else {
      midiCCOut(CCuniDetune, voice1_detune, 1);
    }

    if (voice2_detune < 1 || voice2_detune > 6) {
      midiCCOut(CCuniDetune, 1, 2);
    } else {
      midiCCOut(CCuniDetune, voice2_detune, 2);
    }

    if (voice3_detune < 1 || voice3_detune > 6) {
      midiCCOut(CCuniDetune, 1, 3);
    } else {
      midiCCOut(CCuniDetune, voice3_detune, 3);
    }

    if (voice4_detune < 1 || voice4_detune > 6) {
      midiCCOut(CCuniDetune, 1, 4);
    } else {
      midiCCOut(CCuniDetune, voice4_detune, 4);
    }

    if (voice5_detune < 1 || voice5_detune > 6) {
      midiCCOut(CCuniDetune, 1, 5);
    } else {
      midiCCOut(CCuniDetune, voice5_detune, 5);
    }

    if (voice6_detune < 1 || voice6_detune > 6) {
      midiCCOut(CCuniDetune, 1, 6);
    } else {
      midiCCOut(CCuniDetune, voice6_detune, 6);
    }
  }
...
String getCurrentPatchData() {
  return patchName + "," + String(vcf_attack) + "," + String(vca_attack) + "," + String(vcf_decay) + "," + String(vca_release) + "," + String(vcf_frequency) + "," + String(osc_2_detune) + "," + String(lfo_frequency) + "," + String(portamento) + "," + String(vca_decay) + "," + String(vcf_sustain) + "," + String(vca_sustain) + "," + String(vcf_release) + "," + String(vcf_resonance) + "," + String(vcf_env_amount) + "," + String(pulsewidth) + "," + String(modulation) + "," + String(oscpwm) + "," + String(osc_2_freq) + "," + String(osc_1_freq) + "," + String(osc_2_fm) + "," + String(vcf_mod) + "," + String(osc_1_wave) + "," + String(osc_2_wave) + "," + String(lfo_wave) + "," + String(osc_1_fm) + "," + String(osc_1_pwm) + "," + String(osc_2_pwm) + "," + String(vcf_track) + "," + String(osc_2_half) + "," + String(sync) + "," + String(crossmod) + "," + String(unison) + "," + String(voice1_detune) + "," + String(voice2_detune) + "," + String(voice3_detune) + "," + String(voice4_detune) + "," + String(voice5_detune) + "," + String(voice6_detune);
}
...
void mergeIncoming() {
  if (usbMIDI.read() && usbMIDI.getType()) {
